    ...
    <>
              ...
              {!!item.featuredImage?.node?.sourceUrl && (
                <div className="relative w-full h-36 md:h-40 overflow-hidden flex-shrink-0 bg-white flex items-center justify-center">
                  <img
                    ...
                    className={
                      isCover
                        ? "absolute inset-0 w-full h-full object-cover transition duration-300 group-hover:scale-[1.02]"
                        : "max-w-full max-h-full object-contain transition duration-300 group-hover:scale-[1.02]"
                    }
                  />
                </div>
              )}
      ...
      {active && (
        <DetailPanel
          ...
          image={active.featuredImage?.node ?? null}
          type="project"
          coverImage={(active as WPPost)?.tags?.nodes?.some((t) => t.name === COVER_TAG) ?? false}
          onClose={close}
        />
      )}
    </>
